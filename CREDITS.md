...
`item-hair-effigy.png` is **not** LPC. It is **Radek's own artwork**, supplied
as a finished 32x32 file and shipped unchanged — see "Radek's item icons"
below for the provenance note that covers all three of them.
...
**Radek's own artwork**, supplied as finished 32x32 files and shipped
unchanged — see "Radek's item icons" below.
...
Three files in `public/` are neither LPC, nor CraftPix, nor procedural. They
were drawn for this project and handed over as finished 32x32 PNGs, shipped
byte for byte as received:

| file | item | mission |
| --- | --- | --- |
| `item-hair-effigy.png` | Hair Effigy | Black Annis, the third door |
| `item-guard-ring.png` | Guard Ring | the Minotaur, the fourth door |
| `item-minotaur-earring.png` | Minotaur's Earring | the Minotaur, the fourth door |

**PROVENANCE STILL TO CONFIRM.** They are recorded here as the project owner's
own work because that is how they arrived, and nothing in this file should say
more than is known. If any of the three came out of a licensed pack instead —
CraftPix or otherwise — say which, and this section gets the pack name, the
licence and the purchase reference the same way every other bought asset in
this document does. That correction is cheap now and expensive after a
release.

None of the three carries a ShareAlike obligation on this reading, which
matters: the LPC creatures above do, and the whole reason the relic icons were
never cut out of an LPC layer is to keep the item table free of it.

Each replaced a procedural stand-in generated by a script in `tools/`. Those
scripts are **deleted**, not left in place. A generator that writes to a path
now holding hand-drawn artwork destroys that artwork the moment anybody runs
the directory, and "do not run this one" is not a safeguard.
