...
## The Knell — `public/fx-*-rune.png`, `public/item-*-rune.png`

Source: **CraftPix.net**, standard (non-Enterprise) licence —
https://craftpix.net/file-licenses/

Two originals: a nine-frame skull impact at 128 x 128 and one 32 x 32 stone
(`Icon38`). Same licence, grant and limit as the spell effects and the Home Isle
buildings above — commercial use and sale of the game are permitted outright,
attribution is a courtesy rather than an obligation, and clause 3.1.1 forbids
feeding these files to a model as training data.

> **These files must not sit in a public repository.** The warning on the spell
> effects applies here and it reaches further than it looks: 1.1.3 forbids
> distributing source files, 1.2.1 forbids redistributing the art in any form
> another end user could take and use, and **a recolour is still the art**. So
> the clause covers the thirty derived sheets in `public/` exactly as it covers
> the ten originals — reducing a skull to 32 px and mapping it onto an elemental
> ramp is a derivative, not a new work. Serving them from Vercel as part of the
> running game is explicitly fine. Git keeps every blob it has ever stored, so a
> file pushed once is not removed by deleting it in a later commit.

The ten originals are **not committed at all** — `tools/fx/src/` is gitignored.
Only the derived sheets in `public/` ship, which is the same split the spell
effects already run on.

Thirty files, generated by `tools/fx/gen_rune_art.py` in this repository:

- `fx-<element>-<tier>-rune.png` — 288 x 32, nine frames in one horizontal
  strip, read by `src/gfx/spellArt.ts` exactly as every other sheet is.
- `item-<element>-<code>-rune.png` — 32 x 32, the stone in the bag.

### What was changed

**The impact was reduced 128 -> 32 and recoloured.** Alpha-weighted box
average, so an edge pixel is the mean of the pixels that had ink in them rather
than a mean against transparent black; alpha comes back out hard, because the
source has no partial alpha anywhere and a 32-px frame with soft edges reads as
fog. Strays with no orthogonal neighbour are dropped — the halo of spikes is
the part that does not survive a 4x reduction, and what reads as a crown at 128
reads as dirt at 32.

**Fifteen tints from one animation.** Luminance is mapped onto a five-stop ramp
per element and tier, so the skull's lit brow and its eye sockets survive into
every variant instead of flattening to two tones. The ramps are *sampled off
the crystal icons already in `public/`* rather than invented, so a Knell is the
colour of the Shard it doubles. Two are nudged: Ember redder and Spark
yellower, because straight off the icons they came out the same orange and
"which element just went off" has to be answerable in one frame.

**The stone was tinted and a mark cut into it.** Five marks — flame,
snowflake, mountain, bolt, cyclone — drawn at 8x and thresholded rather than
typed out as pixel maps, so their symmetry cannot drift when one is edited. The
stone body is held to the ramp's middle: a Knell should read as carved rock in
the bag and save the lurid colour for the strip. Each mark gets a one-pixel
groove under it, which is the whole reason one glyph works on all fifteen
stones — a bright mark alone vanishes on a pale Frost, a dark one vanishes on a
near-black Pyre, and a mark with a shadow under it survives both.

Wind takes a spiral rather than the three hooked streaks of the weather glyph.
Below about twenty pixels each hook either merges with its own line or runs off
the box; a spiral is one stroke all the way round, and this element's tiers are
Zephyr, Squall and Cyclone anyway.

## The utility runes — `public/item-{heal,haste,mire,aegis,fury}-rune.png`

Source: **CraftPix.net**, standard (non-Enterprise) licence —
https://craftpix.net/file-licenses/

Three 32 x 32 originals: a rounded nugget (`Icon8`), a faceted gem (`Icon12`)
and a flat slab (`Icon17`). Same licence, grant and limit as the Knells and the
spell effects above — **including the public-repository warning, which reaches
the recolours as well as the originals.** The three are not committed;
`tools/fx/src/` is gitignored.

Five files, generated by `tools/fx/gen_rune_art.py` alongside the Knells.

### What was changed

**Grey, and not by accident.** Fifteen elemental ramps have between them taken
red, blue, pale blue, amber, violet, magenta, navy, silver and dark brown. What
is left unclaimed is grey, green and teal — so these five are grey rock with a
coloured mark, which is exactly what Life and Recall already are. The family
reads as the older, plainer shelf it belongs to, and the mark carries the
meaning rather than the colour: greens and teals for the four that help you,
acid yellow-green for the one that does not.

**The marks are drawn, not sourced.** Cross, triple chevron, hourglass,
shield, three claw slashes — the same 8x-and-threshold construction as the
elemental glyphs, with the same one-pixel groove underneath, so one mark stays
legible on a pale nugget and on the near-black slab alike. The shield is drawn
as an outline: filled solid at fifteen pixels it reads as a gemstone.

**Bodies are assigned by role, not by looks.** Mending and Aegis share the
nugget, Swiftness and Mire share the gem, and Fury gets the slab to itself —
it is the one-off, the expensive one, and the only one with a drawback, so it
is the only one that does not look like anything else in the bag.

## The crystal auras — `public/fx-aura-*.png`

Source: **CraftPix.net**, standard (non-Enterprise) licence —
https://craftpix.net/file-licenses/

A sixteen-frame "Leaf Shield" particle animation at 720 x 720. Same licence,
grant and limit as everything else on this page, **public-repository warning
included, and it reaches the recolours**. Not committed; `tools/fx/src/` is
gitignored.

Six files, generated by `tools/fx/gen_rune_art.py`, each 256 x 32 — eight
frames in one horizontal strip. Five for the utility crystals and a sixth,
lilac, for the Recall Crystal, which had shipped years earlier with no visual
of any kind.

### What was changed

**Sixteen frames became eight.** Frames 9-16 of the source are a byte-identical
copy of 2-8 with only frame 1 differing, so the loop is eight long and shipping
sixteen would have doubled every file to say the same thing twice.

**720 became 32, from a centred 560-pixel crop.** The motes range over roughly
the middle 560 of the frame; reducing the raw 720 would have spent a quarter of
every tile on empty margin.

**Coloured by ALPHA, not by the source's own colour** — the opposite of how the
Knells were done, and the exception exists for a concrete reason. The source
draws its motion trails as near-white at low opacity, so mapping luminance the
usual way sent every trail to the top of the ramp and all five palettes came out
with the same grey smear through them. Alpha already encodes what the ramp wants
to know — faint edge or solid core — so it drives the colour directly, and the
Fury aura ends up with no grey in it anywhere.

**Six colourways, and Mending is not green.** Protective keeps the leaf green
it was drawn as; Fury is reds into violet and is the only one of the five with
neither a green nor a blue in it, which is the point. The big heal takes pale
gold rather than a second green, because Protective and Mending would otherwise
be two green flickers around the same character.

Alpha is kept SOFT here, unlike every other generated sheet in this repo. The
Knell is hard-edged pixel art and had to stay that way; an aura is light around
a character, and light with a one-bit edge reads as a sticker.
...
`item-hair-effigy.png` is **not** LPC. It is **Radek's own artwork**, drawn for
this game and shipped unchanged — see "Relic and jewellery icons" below, where
it is the one of the three relic icons that carries no third-party terms at
all.

It replaced a procedural stand-in that was generated by a script in `tools/`.
That script has been deleted rather than kept: a generator whose output path
now holds hand-drawn artwork is a loaded gun, and the first person to run the
whole `tools/` directory would overwrite the icon with a drawing of it.

The mask trick that produced the redcap's cap and Kárr's helm was no use for
this one either way — the effigy is not a thing the character is wearing, so
there is no layer to stencil it out of. What the icon draws is the decoy
itself: the plaited black hair, the red of the leathers, and a green face,
which is the county's "livid blue" read one turn round the colour wheel.
...
`item-guard-ring.png` and `item-minotaur-earring.png` are **not** LPC. Both are
**CraftPix.net**, standard licence, shipped as purchased — see "Relic and
jewellery icons" below for the pack names and the public-repository warning
that comes with them.

Both replaced procedural stand-ins, and both of those scripts are deleted for
the reason given under the effigy above.

The earring's icon settled a question the item text had left open. The stand-in
drew the ring with a scrap of hide on it; the real one draws **the ear** — the
whole torn lobe, in the boss's own fur-tan, with the bronze inside it. That is
the better reading and the item comment in `src/items.ts` now says so, because
what the player carries to Chronos is not jewellery somebody lost. It is the
part of the animal that proves somebody had been handling it.

The two are also unmistakable side by side in a bag at twelve pixels, which
matters because they come out of the same chest: the ring is a banded
three-quarter hoop in cold teal and bone, and the ear is a tall warm curl with
no straight edge anywhere on it.
...
## Relic and jewellery icons — `item-hair-effigy.png`, `item-guard-ring.png`, `item-minotaur-earring.png`

Three 32x32 item icons, from two different places, and the split matters
because only one of the two carries obligations.

### The effigy — Radek's own work

`item-hair-effigy.png`, the relic Black Annis leaves, was **drawn by the
project owner for this game**. Not LPC, not CraftPix, not procedural: no
attribution owed to anybody, no ShareAlike, no DRM clause, and nothing in this
file has to travel with it.

What it draws is the decoy itself — plaited black hair, the red of the
leathers, and a green face, which is the county's "livid blue" read one turn
round the colour wheel.

### The ring and the ear — CraftPix
...
| file | item | pack |
| --- | --- | --- |
| `item-guard-ring.png` | Guard Ring | `craftpix-net-539335-ring-earring-and-amulet-pixel-art-icons` |
| `item-minotaur-earring.png` | Minotaur's Earring | `craftpix-net-530890-chaos-monster-loot-32x32-icons` |

The assignment above is read off the artwork: the ring is a ring and comes from
the jewellery pack, and the earring's icon is not a ring at all but a torn ear,
which is monster loot. If the two are in fact the other way round in the
purchase, swap the rows — it is one edit and nothing in the code depends on it.

Same terms as the dragon and the buildings: commercial use granted outright, no
royalty, no attribution owed on a paid product, and the same clause 1.1.3 /
1.2.1 restriction on public source files. **The public-repository warning in
the buildings section applies to these two files in full.** Both ship exactly
as purchased — no recolour, no rescale, no redraw; the only change made was
re-saving the ring from an indexed palette to RGBA so it matches the other
icons in the folder byte-format-wise.

### They replaced procedural stand-ins, and those scripts are deleted

All three shipped first as drawings generated by scripts in `tools/`. Those
scripts are **gone**, not left in place with a warning. A generator whose
output path holds bought or hand-drawn artwork destroys that artwork the first
time anybody runs the directory, and "do not run this one" is not a safeguard.
...
it became the Hunter set, and settled on a green-and-brown forest look: a
forest-green hood, a green jerkin with brown panels and tan lacing, green
leggings over brown wraps, green shoes on brown soles. Colour for colour,
under the adaptation right of clause 1.1.1, with two things **drawn in by
hand** in the same outline: a red feather tucked into the hood, and a quiver
strap with a gold buckle across the jerkin. The files keep their snakeskin
names, as the item ids do.
