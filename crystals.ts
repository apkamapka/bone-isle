...
import { HEAL_CRYSTAL_BASE, FIRE_CRYSTAL_DMG, FIRE_CRYSTAL_RANGE } from "../config.ts";
...
export const CRYSTAL_KINDS: readonly ItemKind[] = ["healCrystal", "fireCrystal", "recallCrystal"];
...
/**
 * Apply a Life or Fire crystal. Returns true if a charge was consumed.
...
export function useCrystal(world: World, p: Player, kind: ItemKind): boolean {
  ...
  if (kind === "fireCrystal") {
    ...
    removeItem(p.bag, kind, 1);
    const dmg = FIRE_CRYSTAL_DMG + p.level;
    best.hp -= dmg;
    best.hurtT = 0.2;
